...
import { AzureResource, CostOptimization, CostOptimizationSuggestion } from '../types';
...
export class CostOptimizationAgent {
  ...
  private async callAzureAIFoundryAgent(payload: any): Promise<any> {
    logger.info('Calling Azure AI Foundry Cost Optimization Agent', {
  ...
  private async getCurrentPricing(region: string): Promise<Record<string, number>> {
  ...
  async generateDetailedCostReport(
...
`;
  ...
  private estimateResourceCost(resource: AzureResource, region?: string): number {
  ...
  private getCostCategory(cost: number): string {
    ...
    if (cost < 100) return 'Medium Cost';
    if (cost < 300) return 'High Cost';
    return 'Very High Cost';
  }

  private calculateEfficiencyScore(result: CostOptimization): number {
  ...
  private getOptimizationPriority(savings: number, impact: string): string {
    if (savings > 50 || impact === 'high') return 'HIGH';
    if (savings > 20 || impact === 'medium') return 'MEDIUM';
    return 'LOW';
  }

  private getImplementationSteps(suggestion: string): string {
  ...
  private generateRegionalComparison(baseCost: number, currentRegion?: string): string {
    ...
    return regions.map(region => {
      const cost = baseCost * (multipliers[region as keyof typeof multipliers] || 1.0);
      const current = region === currentRegion ? ' (Current)' : '';
      return `- **${region}${current}**: $${cost.toFixed(2)}/month`;
    }).join('\n');
  }

  private parseOptimizationResult(result: any): CostOptimization {
    const optimizations: CostOptimizationSuggestion[] = result.optimizations.map((opt: any) => ({
